...
const FinancialReport = () => {
  ...
  const fetchData = async () => {
    const records = await pb.collection("financial_results").getFullList(
      {
        sort: "sno",
      },
      { requestKey: null }
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingRow) {
        ...
        record = await pb.collection("financial_results").create(updateData);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      await pb.collection("financial_results").delete(editingRow.id);
  ...
  return (
    <>
      ...
      <div className="fixed top-0 p-4 bg-gray-100 border-b w-full flex flex-wrap gap-2 z-10">
        ...
        <span className="text-black">Financial Report</span>
      </div>
          ...
          <div className="relative">
            ...
            >
              <h3 className="text-lg font-semibold mb-4">
                {editingRow ? "Edit" : "Add"} Financial Report
              </h3>
